...
import LoanApplicationListTable from "../components/LoanApplicationListTable";
...
const PAGE_TITLE = "Daftar Pengajuan Pinjaman";
export default {
  ...
  setup(_, { root }) {
    ...
    watch(toRef(uploadDocument, "modalActive"), isActive => {
      if (!isActive) {
        ...
        uploadDocument.loanApplication = null;
        uploadDocument.documentType = null;
      }
    });
    const handleUploadDocument = loanApplication => {
      uploadDocument.file = null;
      uploadDocument.documentType = null;
      uploadDocument.loanApplication = loanApplication;
    ...
    const doUploadDocument = () => {
      ...
      isBusy.value = true;
      const url = `/banks/${selectedBankId.value}/loan-apps/${uploadDocument.loanApplication.id}/documents`;
      apiInvoker
        .post(url, data, {
        ...
        .then(() => {
          ...
          Swal.fire(
            ...
            `Dokumen untuk Pengajuan Pinjaman ${uploadDocument.loanApplication.id} berhasil diunggah!`,
            "success",
          );
    ...
    watch(toRef(confirmStatus, "modalActive"), isActive => {
      if (!isActive) {
        ...
        confirmStatus.loanApplication = null;
      }
    });
    const handleConfirmStatus = ({ loanApplication, status }) => {
      confirmStatus.status = status;
      confirmStatus.loanApplication = loanApplication;
      confirmStatus.modalActive = true;
    };
    const doConfirmStatus = () => {
      const reqData = { status: confirmStatus.status };
      isBusy.value = true;
      const url = `/banks/${selectedBankId.value}/loan-apps/${confirmStatus.loanApplication.id}/status`;
      apiInvoker
        .patch(url, reqData, {
        ...
        .then(res => {
          ...
          Swal.fire(
            ...
            `Pengajuan Pinjaman ${
              confirmStatus.loanApplication.id
    ...
    const refreshData = () => {
      ...
      const url = `/banks/${selectedBankId.value}/loan-apps`;
      isBusy.value = true;
      apiInvoker
