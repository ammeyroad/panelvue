        ...
        <td>
          <ValidationProvider v-slot="{ errors }" rules="required">
            <b-field
              ...
              :type="errors.length > 0 ? 'is-danger' : null"
            >
              <b-select placeholder="Pilih Periode" :disabled="disabled">
...
export default {
  name: "DepositoInterestRatesForm",
  ...
  props: {
    ...
    value: {
      ...
      validator(v) {
        ...
        const requiredKeys = ["min", "max", "period", "interest"];
        return every(v, rate => {
          let isValid = true;
  ...
  methods: {
    ...
    handlePeriodInput(index, newValue) {
      const values = cloneDeep(this.interestRates);
      values[index].period = newValue;
      this.updateValue(values);
    },
